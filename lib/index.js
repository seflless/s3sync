// Work around for this issue with bucket names with
// '.' in them. See: https://github.com/awssum/awssum/issues/164
...
function shouldIgnoreFileName(config,fileName,op){
  ...
  // Ignore .metadata/ folder files automatically
  // TODO: There may be some libs/frameworks that use 
  // .metadata folders, but it's unlikely to matter
  // for all of our use cases
  if(fileName.search('.metadata/')!=-1){
    return true;
  }
...
function getFileMD5(file,cb){
  ...
  s.on('error', function() {
      console.log('Error creating the MD5 for: '+file);
  });
}
